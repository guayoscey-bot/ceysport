...
let todosLosProductos = [], catAct = '', subFiltroAct = '', precioFiltro = 'todos', productoSeleccionado = {};

async function cargar() {
    try {
        // Decap CMS genera los archivos en una ruta estática. 
        // Hacemos un fetch a una lista o archivo de índice de productos.
        ...
        // Fallback visual si aún no hay productos creados en el admin
        document.getElementById('lista').innerHTML = '<p style="color:white; text-align:center;">No hay productos creados todavía. Entra a /admin para agregar el primero.</p>';
    }
}
...
function aplicarFiltros() {
    ...
    if (subFiltroAct) {
        filtrados = filtrados.filter(p => (p.Marca?.toLowerCase().trim() === subFiltroAct.toLowerCase().trim() || p.Tipo?.toLowerCase().trim() === subFiltroAct.toLowerCase().trim()));
    }
...
function renderizar(prods) {
    ...
    lista.innerHTML = prods.length === 0 ? '<p style="color:white; text-align:center;">No hay productos con estos filtros.</p>' : prods.map(p => `
        <div class="card">
            <img src="${p.imagen_url}" alt="${p.nombre}">
            <h3>${p.nombre.toUpperCase()}</h3>
            <p class="desc-text">${p.descripcion || ''}</p>
            <p class="price-text">$${parseInt(p.precio).toLocaleString('es-CO').replace(/,/g, ".")}</p>
            <select class="tallas-select" id="talla-${p.id}">${(p.tallas ? p.tallas.split(',') : ['Única']).map(t => `<option value="${t.trim()}">${t.trim()}</option>`).join('')}</select>
            ...
            <button class="btn-action" onclick="abrirPedido('${p.id}', '${p.nombre.replace(/'/g, "\\'")}', '${p.imagen_url}', 'talla-${p.id}')">PEDIR POR WHATSAPP</button>
        </div>`).join('');
}

function abrirPedido(id, nombre, img, tallaId) {
    productoSeleccionado = { nombre, img, talla: document.getElementById(tallaId).value };
